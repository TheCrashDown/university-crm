...
from routers.pydantic_models import InsertQuizForm, InsertQuizQuestionsForm, InsertQuizAnswersForm
...
def quiz_insert(quiz: InsertQuizForm, teacher_name:str):
    with Util.get_session() as session:
        ...
        session.execute(insert(Quiz).values(name=quiz.name,
                                            teacher=data['id']))
        session.commit()
    return {"success": True}
...
def quiz_question_insert(question: QuizQuestions, quiz_name:str):
    with Util.get_session() as session:
        ...
        session.execute(insert(Quiz).values(name=question.question,
                                            quiz = data['id']))
        session.commit()
    return {"success": True}
